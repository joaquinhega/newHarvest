...
// --- CONFIGURACIÓN DE DATOS DESDE LA BASE DE DATOS ---
if (!isset($_POST['id'])) {
...
if (!empty($row['id_empresa'])) {
    ...
    $sqlEmpresa = "SELECT path FROM empresa WHERE id_empresa = '$id_empresa' AND borrado = 0";
    $resEmpresa = $conn->query($sqlEmpresa);
    if ($resEmpresa && $empresaRow = $resEmpresa->fetch_assoc()) {
        if (!empty($empresaRow['path']) && file_exists('../' . $empresaRow['path'])) {
            $logo_empresa = '../' . $empresaRow['path'];
        }
    }
}
...
class VoucherPDF extends FPDF
{
    ...
    function Header()
    {
        // Logo New Harvest (izquierda)
        $this->Image('../assets/logo-newHarvest-negro.png', 18, 13, 45);
        // Logo empresa (derecha, si existe)
...
$block_width = 18;
$block_height = 12;
$block_gap = 0; // Sin espacio entre bloques
...
foreach ($fecha_parts as $i => $parte) {
    ...
    // No agregar espacio entre bloques
}
$pdf->Ln(18);
...
foreach ($labels as $label => $value) {
    ...
    $pdf->SetTextColor(0,0,0);
    $pdf->Cell(90, 10, utf8_decode($value), 0, 1, 'L');
    $pdf->SetFont('Arial', 'B', 13);
    $pdf->SetTextColor(255,255,255);
    $y += 13;
}
...
if (!empty($row['Firma']) && file_exists($row['Firma'])) {
    $pdf->Image($row['Firma'], 195, 105, 40); // ancho 40mm
}
